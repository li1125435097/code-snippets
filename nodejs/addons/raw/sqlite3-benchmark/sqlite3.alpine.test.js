// main.js
const { OpenDb, Exec} = require('../sqlite3/build/Release/addon')
const fs = require('fs')
...
// const query = (sql,cb=new Function()) => Exec(sql, cb)
const query = Exec
...
async function main() {
    ...
    // 测试插入时间
    console.time('insert')
    for (let sql of insertsql) await query(sql)
    console.timeEnd('insert')

    // 测试批量插入时间
    console.time('insert multi')
    await query(insertsql2)
    console.timeEnd('insert multi')

    // 测试查询时间
    console.time('query')
    let result = await query(querysql,(a,b) => console.log(b))
