...
template <typename T>
void printIter(T& unordered_map) {
    cout << "[ ";
...
int main() {
    ...
    // 集合构造
    unordered_map<int,int> st1; // 定义一个空的容器
    unordered_map<int, int> st2({{11,1}, {22,2}, {33,3}, {44,4}}); // 4个

    // 迭代器构造
    pair<int,int> arr[]= {{1, 2}, {3, 4}, {5, 6}};
    unordered_map<int,int> st3(arr, arr + 3); // 3个

    // 拷贝构造
    unordered_map<int,int> st4(st2); // 3个
    ...
    // 遍历所有桶,哈希策略观察
    for (size_t i = 0; i < st4.bucket_count(); ++i) {
        int size = st4.bucket_size(i);
        if(size > 0){
            cout << "Bucket " << i << " size: " << size << endl; 
        }
    }
